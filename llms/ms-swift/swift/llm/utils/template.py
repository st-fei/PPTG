...
# 对所有支持模型的模板类
    # 定义一些对话结构的参数，支持多轮对话、多模态输入等
    # 通过这些属性和方法，Template类能够为模型提供统一的输入模板格式，适应不同的任务需求
class Template:
    ...
    # 支持多模态任务的特殊token与键
    # special_tokens用于标记多模态输入中的特殊token
    special_tokens = ['<image>', '<video>', '<audio>', '<bbox>', '<ref-object>']
    # special_keys表示与special_tokens对应的键
    special_keys = ['images', 'videos', 'audios', 'objects']
    # grounding_type表示用于多模态任务的归一化方法
    grounding_type = 'norm_1000'
    # image_placeholder表示图像占位符
    image_placeholder = ['<image>']
    # load_medias表示是否加载多模态数据
    load_medias = True
    compute_per_round_loss = True  # for rlhf
    output_prompt_answer = False  # for encoder-decoder & kto

    def __init__(self,
                 # prefix：对话开始前的前缀token
                 prefix: Prompt,
                 # prompt：用户的对话提示
                 prompt: Prompt,
                 # chat_sep：每轮对话之间的分隔符
                 chat_sep: Optional[Prompt],
                 # suffix：对话结束时的后缀token
                 suffix: Prompt,
                 # default_system：系统的默认行为提示（如设定：你是一个得力的助手）
                 default_system: Optional[str] = None,
                 # system_prefix：系统行为的前缀token
                 system_prefix: Optional[Prompt] = None,
                 # auto_add_bos：是否自动添加<bos> token
                 auto_add_bos: bool = False,
                 # tools_prompt：工具提示相关的配置
                 tools_prompt: str = 'react_en',
                 # tool_prompt：工具提示的相关内容
        ...
        # 如果default_system为空字符串，则设置为None
        if default_system == '':
            default_system = None
        # 检查prefix中是否包含系统行为，如果包含，则system_prfix必须为空，因为prefix已经包含
        if self._has_system(prefix):
            assert system_prefix is None, 'The prefix already contains {{SYSTEM}}.'
            system_prefix = prefix
            # 随后，系统行为被从prefix中替换掉，防止重复
    ...
    @staticmethod
    # _preprocess_prompt函数用于处理对话模板中的Prompt
        # 该功能将eos_token_id等字符串标识符转换为实际token id
