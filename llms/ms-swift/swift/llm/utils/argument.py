...
class ArgumentsBase:
    ...
    # 将key对应的json文件路径转换为实际值
    # raw: key: path(str)
    # post: key: json.load(path)
    ...
    # post_init方法主要目的为在类实例化后，执行一些额外的初始化逻辑
    ...
    # 验证路径的存在性，并将路径标准化为绝对路径
        # 如果路径不存在，则抛出FileNotFoundError异常
    @classmethod
    def _check_path(cls,
    ...
    # 对给定的一组路径属性进行检查和标准化，并根据路径的类型决定是否进行存在性检查
        # 其可以处理不同类型的参数类（如SftArguments和InferArguments）
        # 根据这些类的特定字段来决定哪些路径需要检查
    def handle_path(self: Union['SftArguments', 'InferArguments']) -> None:
        # 定义需要检查的路径(check_exist_path包含必须存在的路径)
        check_exist_path = ['ckpt_dir', 'resume_from_checkpoint', 'custom_register_path']
        # maybe_check_exist_path包含可能需要检查的路径
        ...
        # 决定哪些maybe_check_exist_path应该检查
    ...
    # 检查模型是否支持Flash Attention
        # 若用户启用Flash Attention但模型不支持，则在日志发出警告
    ...
    def handle_generation_config(self: Union['SftArguments', 'InferArguments']) -> None:
        # 如果温度系数设置为0，则do_sample=False
        if self.temperature == 0:
            self.do_sample = False
        # 如果do_sample设置为False:
            # 如果当前为InferArguments且推理后端为pytorch 或 当前为SftArguments
            # 自动调整一些生成超参数
    ...
    def handle_compatibility(self: Union['SftArguments', 'InferArguments']) -> None:
        ...
        # dataset_name_mapping定义数据集名称的映射
            # 将某些特定数据集名称转换为标准化名称或细化子集的数据集
        ...
        # 依次检查self.template_type和self.model_type，如果其出现在映射中，则将其转换为映射值
        ...
        for key in ['dataset', 'val_dataset']:
            ...
            # 检查该数据集名称是否在dataset_mapping中，如果是则进行转换
        ...
        # 如果设置了自定义数据集，则将其添加到self.dataset中
    ...
    # 用于处理custom_dataset_info参数
        # 如果该参数不为空，则调用register_dataset_info_file()进行自定义数据集注册
    ...
    def _handle_dataset_sample(self: Union['SftArguments', 'InferArguments']):
        ...
        # 如果self.dataset长度不为1或self.train_dataset_sample == -1(即未设置)，则直接返回
        ...
        # 对样本数标记作出修正
        # 若train_sample == -1，则表示数据集名称未指定样本数，此时使用self.train_dataset_sample来指定样本数
        if train_sample == -1:
            train_sample = self.train_dataset_sample
        # 否则，说明该数据集已在#后指明样本数，在train_sample和self.train_sample中取较小样本数
        else:
    ...
    def _handle_dataset_compat(
        ...
        # 根据self.dataset_seed初始化随机数生成器
        random_state = np.random.RandomState(self.dataset_seed)
        # 获取验证集采样的配置，用于决定是否对验证集进行子集选择
        val_dataset_sample = self.val_dataset_sample
        # 如果存在train_dataset且self.train_dataset_sample >= 0
        # self.train_dataset_sample用于指定训练集数量，因此如果train_dataset数量大于self.train_dataset_sample，则采样
    ...
    # 用于从指定的检查点目录(ckpt_dir)中加载先前保存的配置参数(sft_args.json或export_args.json)
    # 将上述参数导入到当前对象实例中（根据是否为sft来决定加载哪些参数）
    # 用于恢复训练或推理时的配置
    def load_from_ckpt_dir(self, is_sft: bool = False) -> None:
        # 根据是否为sft过程来确定加载的检查点目录
        if is_sft:
            ckpt_dir = self.resume_from_checkpoint
        else:
            ckpt_dir = self.ckpt_dir
        # 检查sft_args_path和export_args_path路径是否存在
        ...
        if not os.path.exists(sft_args_path) and not os.path.exists(export_args_path):
            ...
            return
        # 加载json文件（主sft_args_path后export_args_path)
        ...
        for key in imported_keys:
            ...
            if old_value is None:
                continue
            # 如果当前参数为dataset或val_dataset，且当前属性已有值，则跳过
            ...
            if key in {'template_type', 'dtype'} and value != 'AUTO':
                continue
            # 将key对应的属性值恢复成从断点文件中加载值相同
            setattr(self, key, old_value)

        # compat
        # 兼容性处理，如果self.val_dataset为None，将其设置为空列表
...
class InferArguments(ArgumentsBase):
    ...
    # add by stf 2024.12.9
    img_dir: Optional[str] = field(default="/data/tfshen/pcg_img_sample", metadata={'help': '/path/to/your/img_dir'})

    load_args_from_ckpt_dir: bool = True
    load_dataset_config: bool = False
    eval_human: Optional[bool] = None
    # add by stf 2024.11.14
    eval_ficl: Optional[bool] = False # 是否在代码中使用FICL推理评估
    ficl_sample_dataset: Optional[str] = None # ficl采样数据文件路径
    ficl_model_gen_path: Optional[str] = None # 存储所有模型推理结果的文件路径
    ficl_save_dir: Optional[str] = None # 存储ficl结果文件夹
    ficl_example_num: int = 0 # FICL的默认示例数量
    use_ficl_history_title: Optional[bool] = False # 是否启用历史标题
    use_ficl_history_img: Optional[bool] = False # 是否启用历史图像
    use_ficl_target_img: Optional[bool] = False # 是否启用目标图像

    # add by stf 2024.11.20 [Unified Direct Comparison Framework - udcf]
    eval_udcf: Optional[bool] = False # 是否在代码中使用UDCF推理评估
    udcf_mode: Optional[str] = 'dataset' # 执行<test>模式 or <dataset>模式
    enable_udcf_cot: Optional[bool] = False # 是否启用udcf cot
    enable_udcf_streamline: Optional[bool] = True # 是否在udcf中使用简化版样例
    udcf_sample_dataset: Optional[str] = None # udcf采样数据文件路径
    udcf_model_gen_path: Optional[str] = None # 存储所有模型推理结果的文件路径
    udcf_save_dir: Optional[str] = None # 存储udcf结果文件夹
    udcf_rank_num: int = 4 # udcf@num
    udcf_example_num: int = 0 # udcf示例数量
    use_udcf_history_img: Optional[bool] = False # 是否启用历史图像
    
    # add by stf 2024.11.27 [指定消融评估时的消融模型名以及消融模型推理结果路径]
    aba_model_name: Optional[str] = None # 消融评估模型名
    aba_model_gen_path: Optional[str] = None # 存储消融模型推理结果

    # add by stf 2024.11.20 [cot]
    enforce_cot: Optional[bool] = False # 是否执行cot指令
    cot_mode: Optional[str] = 'dataset' # 执行<test>模式 or <dataset>模式
    is_sample: Optional[bool] = True # 是否执行采样数据
    control_memory_bank: Optional[bool] = False # 是否执行控制记忆块版本
    inherient_gen_file: Optional[str] = None # 继承的生成文件地址
    cot_streamline: Optional[bool] = True # 使用<简化图像> or <原始图像>
    use_cot_history_img: Optional[bool] = False # 是否启用历史图像 (仅用于未适配记忆版本)
    enable_visible_img: Optional[bool] = False # visible-parse 是否启用历史图像
    enable_hidden_img: Optional[bool] = False # hidden-parse 是否启用历史图像
    cot_sample_dataset: Optional[str] = None # cot采样数据文件路径
    cot_save_dir: Optional[str] = None # 存储cot结果文件夹

    # add by stf 2024.11.29
    enforce_transfer: Optional[bool] = False # 是否执行GPT-4o Transfer指令
    transfer_mode: Optional[str] = 'dataset' # 执行<test>模式 or <dataset>模式
    transfer_deque_dir: Optional[str] = None # 存储待transfer文件文件夹
    transfer_save_dir: Optional[str] = None # 存储transfer结果文件夹

    # add by stf 2025.1.2
    enforce_refinement: Optional[bool] = False # 是否执行纯文本模型的润色
    refinement_mode: Optional[str] = 'dataset' # 执行<test>模式 or <dataset>模式
    refinement_inherient_file: Optional[str] = None # 润色前标题文件
    refinement_sample_dataset: Optional[str] = None # 采样数据文件
    refinement_save_dir: Optional[str] = None # 存储refinement结果文件夹
    

    # add by stf 2024.11.21 组索引
    group_index_path: Optional[str] = None
