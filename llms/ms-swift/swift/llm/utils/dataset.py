...
def register_dataset(dataset_name: str,
    ...
    if get_function is not None:
        # 检查function_kwargs
            # 如果function_kwargs包含参数，创建一个带有传入function_kwargs参数的新get_function函数
        if len(function_kwargs) > 0:
            get_function = partial(get_function, **function_kwargs)
        # 将get_function函数加入dataset_info中
...
def register_local_dataset(
    ...
    # 如果传入的base_dir不为None，则将dataset_path中的相对路径转换为绝对路径
...
# 用于解析一个数据集的名称字符串
# 从中提取出：
    # use_hf：是否使用Huggingface数据集
    # dataset_name：数据集的实际名称
    # subset_list：子集列表
    # dataset_sample：数据集样本数量
def parse_dataset_name(dataset_name: str) -> Tuple[bool, str, List[str], int]:
    ...
    # 尝试从dataset_name中分离出是否使用Huggingface(use_hf)和其余部分(other)
    use_hf, other = _safe_split(dataset_name, '::', False)
    # 如果use_hf为None，则从环境变量USE_HF中读取
    if use_hf is None:
        use_hf = strtobool(os.environ.get('USE_HF', 'False'))
    # 如果use_hf为字符串，则根据其值将其转换为布尔值
...
# 将数据集ID或路径转化为数据集名称
    # 通过查找预定义的映射表DATASET_MAPPING，将数据集ID或路径与数据集名称相互关联
    # 如果找不到匹配的数据集名称，会注册新的数据集信息
def _dataset_id_to_name(dataset_name_list: List[str]) -> List[str]:
    # register dataset_id (ms/hf). Convert dataset_id to dataset_name.
    # 初始化映射表
    ms_dataset_mapping = {} # modelscope dataset mapping
    hf_dataset_mapping = {} # huggingface dataset mapping
    ### 以下注释以dataset_id_or_path & ms_dataset_mapping为例 ###
    ...
    for i, (d, use_hf, d_id_or_path) in enumerate(extra_dataset):
        ...
        # 调用register_dataset_info注册新数据集（提供d_name和d_info)
        # import pdb; pdb.set_trace()
        register_dataset_info(d_name, d_info)
        # 将注册后的数据集添加至res_dataset
        res_dataset.append(d.replace(d_id_or_path, d_name))
    # 返回res_dataset
    return res_dataset

# 根据给定的参数，从多个数据源（HuggingFace或ModelScope）获取训练集和验证集
    # 根据数据集的名称、种子、采样比例以及其他选项加载、处理和返回训练集和验证集
def get_dataset(
        # dataset_name_list: 数据集名称
        dataset_name_list: Union[List[str], str],
        # 划分验证集的比例，默认为0.0（即没有验证集）
        dataset_test_ratio: float = 0.,
        # 数据集种子
        dataset_seed: Union[int, RandomState] = 42,
        # 检查数据集的策略（支持none、discard、error、warning）
    ...
    # 将给定的数据集ID或路径转换为标准的数据集名称
    dataset_name_list = _dataset_id_to_name(dataset_name_list)
    # 遍历数据集
    for dataset_name in dataset_name_list:
        # 使用parse_dataset_name函数解析数据集名称、子集和采样大小
        use_hf, dataset_name, subsets, dataset_sample = parse_dataset_name(dataset_name)
        # 从DATASET_MAPPING中获取数据集的元信息，包含：
            # get_function: 获取函数
            # is_local: 是否为本地数据集
            # dataset_id_or_path: 数据集路径
            # remove_useless_columns: 是否需要移除无用列
        # DATASET_MAPPING结构为Dict[str, Dict[str, Any]]
        ...
        # 如果get_function返回的是(list, tuple)类型，则代表有验证集
        ...
        # 根据返回结果，将训练集和验证集分别加入train_dataset_list和val_dataset_list
...
def get_local_dataset(_1: str,
    ...
    streaming = kwargs.get('streaming', False)
    # TODO 以下这行报错
        # split: ['/home/tfshen/pyproject/pcg/llms/ms-swift/dataset/pcg/processed/test.jsonl']
        # preprocess_func: SmartPreprocessor()
        # streaming: False
    # import pdb; pdb.set_trace()
